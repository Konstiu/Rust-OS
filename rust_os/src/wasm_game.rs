...
pub fn update_game() {
    if let Some(game_mutex) = WASM_GAME.get() {
        ...
        let update_fn = game.game_update.typed::<(), ()>(&game.store)
...
pub fn handle_key(key_code: u8) {
    if let Some(game_mutex) = WASM_GAME.get() {
        let mut game = game_mutex.lock();
        let handle_key_fn = game.handle_key.typed::<i32, ()>(&game.store)
...
pub fn render_game() {
    if let Some(game_mutex) = WASM_GAME.get() {
        let mut game = game_mutex.lock();
        let render_fn = game.game_render.typed::<(), ()>(&game.store)
...
pub fn handle_direction(direction: i32) {
    if let Some(game_mutex) = WASM_GAME.get() {
        let mut game = game_mutex.lock();
        let set_dir_fn = game.set_direction.typed::<i32, ()>(&game.store)
...
fn register_framebuffer_functions<T>(linker: &mut Linker<T>) {
    ...
    linker
        .func_wrap("env", "get_framebuffer_width", 
    ...
    linker
        .func_wrap("env", "get_framebuffer_height", 
            |_caller: Caller<T>| -> i32 {
    ...
    // get_grid_width() -> i32
    linker
        .func_wrap("env", "get_grid_width", 
            |_caller: Caller<T>| -> i32 {
                framebuffer::grid_size()
                    .map(|(w, _)| w as i32)
                    .unwrap_or(0)
            })
        .unwrap();
    ...
    linker
        .func_wrap("env", "init_cell_size", 
            |_caller: Caller<T>, size: i32| {
